...
import { TrendingUp, TrendingDown, Minus, Search, RefreshCw, BarChart2, X, ExternalLink, ShieldCheck, MapPin, ChevronLeft, ChevronRight } from 'lucide-react'
  ...
  return (
    <section className="flex h-full flex-col rounded-3xl border border-border bg-card p-5 lg:p-6 shadow-sm">
      ...
      {!isLoading && totalPages > 1 && (
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-border pt-4 text-sm text-muted-foreground">
          <div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 rounded-lg border border-border px-3 py-1.5 font-bold text-foreground hover:bg-secondary disabled:opacity-50 transition-colors"
            >
              <ChevronLeft className="size-4" /> Prev
            </button>
            <span className="font-extrabold text-foreground mx-2">{currentPage} / {totalPages}</span>
            <button
