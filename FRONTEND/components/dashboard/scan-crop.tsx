  ...
  const handleDiagnose = async () => {
    ...
    try {
      ...
      if (data.status === 'success' && data.data) {
        // Save scan result to Firestore using localStorage session or auth
        const session = localStorage.getItem('krishi_session')
        let userId = auth.currentUser?.uid || 'anonymous'
        if (session) {
          try {
            const user = JSON.parse(session)
            userId = user.phoneOrEmail || userId
          } catch(e){}
        }

        if (data.data.is_plant !== false && userId !== 'anonymous') {
          try {
            ...
            if (data.data.crop) {
              ...
              await addDoc(collection(db, 'users', userId, 'crops'), {
                name: data.data.crop,
                variety: 'Unknown (Scanned)',
                plantDate: today,
                health: data.data.status === 'healthy' ? 'Healthy' : 'Needs attention',
                timestamp: serverTimestamp()
              })
            }
          } catch (e) {
        ...
        // Save the detected crop globally so Lose Less can auto-fill it!
        if (data.data.crop) {
          localStorage.setItem('krishirakshak_active_crop', data.data.crop)
        }
