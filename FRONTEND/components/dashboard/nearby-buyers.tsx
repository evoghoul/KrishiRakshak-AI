...
import { MapPin, BadgeCheck, Phone, MessageSquare, X, Send, CheckCircle2, Building, ShieldCheck, IndianRupee, Sparkles, Loader2 } from 'lucide-react'
...
export function NearbyBuyers() {
  ...
  const handleAiSearch = async () => {
    setIsSearching(true)
    try {
      const res = await fetch('http://localhost:8000/api/ai-buyers?location=Vadlamudi')
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'buyers'), (snapshot) => {
      const loaded = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Buyer))
      setBuyers(loaded)
    })
    return () => unsubscribe()
  }, [])
          ...
          >
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <a
                  href={`https://wa.me/91${selectedBuyer.phone.replace(/[^0-9]/g, '').slice(-10)}?text=Hello%2C%20I%20am%20a%20farmer%20from%20KrishiRakshak%20interested%20in%20selling%20my%20${encodeURIComponent(selectedBuyer.crop)}%20harvest.`}
                  target="_blank"
                  rel="noreferrer"
