...
import { Truck, Tractor, MapPin, Star, IndianRupee, X, CheckCircle2, Phone, ShieldCheck, Navigation, Sparkles, Loader2 } from 'lucide-react'
...
export function LogisticsMatching() {
  ...
  const handleAiSearch = async () => {
    setIsSearching(true)
    try {
      const res = await fetch(`/api/ai-logistics?location=Vadlamudi`)
  ...
  const handleConfirmBooking = async (e: React.FormEvent) => {
    ...
    try {
      const vRef = doc(db, 'vehicles', bookingVehicle.id)
      await updateDoc(vRef, { available: false })
  ...
  return (
    <section className="rounded-3xl border border-border bg-card p-5 lg:p-6 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="rounded-full bg-primary/10 border border-primary/20 px-3 py-1 text-xs font-bold text-primary flex items-center gap-1 whitespace-nowrap">
