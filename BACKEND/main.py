...
async def get_live_prices():
    ...
    if not api_key or len(api_key) < 10:
        return {"status": "error", "error_message": "🚨 DATA_GOV_API_KEY is missing or invalid in your .env file."}
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if not records:
                return {"status": "error", "error_message": "✅ API Connected, but Government portal has 0 records for Andhra Pradesh today."}
            
            live_data = []
            for record in records:
                m_price = record.get("modal_price")
                if m_price:
                    live_data.append({
            ...
            return {"status": "success", "source": "Agmarknet Live", "count": len(live_data), "data": live_data}
            
    except Exception as e:
        return {"status": "error", "error_message": f"🚨 Failed to fetch from Gov API: {str(e)}"}
